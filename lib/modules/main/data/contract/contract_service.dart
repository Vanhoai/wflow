...
class ContractServiceImpl implements ContractService {
  ...
  @override
  Future<HttpResponseWithPagination<ContractEntity>> getContractCompleted(GetContractSigned request) async {
    try {
      final url =
          request.isBusiness ? ContractPaths.getContractCompletedBusiness : ContractPaths.getContractCompletedUser;
      ...
      List<ContractEntity> contracts = [...httpResponseWithPagination.data.map((e) => ContractEntity.fromJson(e))];
  ...
  Future<List<TaskEntity>> uploadFileAddToContact(RequestAddTaskExcel request) async {
    try {
      var formData = FormData.fromMap({
        'excel': await MultipartFile.fromFile((request.file.path)),
        'contract': request.contract,
      });
      final response = await agent.dio.post(
        ContractPaths.uploadFileAddToContract,
        data: formData,
      );
