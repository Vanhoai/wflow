...
class AddBusinessBloc extends Bloc<AddBusinessEvent, AddBusinessState> {
  ...
  Future<void> onInitAddBusiness(InitAddBusinessEvent event, Emitter emit) async {
    ...
    final result = await userUseCase.getUsersNotBusiness(getUserNotBusinessModel);
  ...
  Future<void> onSearchAddBusiness(SearchAddBusinessEvent event, Emitter emit) async {
    ...
    GetUserNotBusinessModel getUserNotBusinessModel = GetUserNotBusinessModel(search: event.txtSearch);
    final result = await userUseCase.getUsersNotBusiness(getUserNotBusinessModel);

    result.fold((List<UserEntity> users) => {emit(state.copyWith(users: users, page: 1))}, (Failure failure) => {});
    instance.get<AppLoadingBloc>().add(AppHideLoadingEvent());
  }
  ...
  Future<void> onScrollAddBusiness(ScrollAddBusinessEvent event, Emitter emit) async {
    ...
    result.fold(
      (List<UserEntity> users) => {
        ...
        emit(state.copyWith(users: newUsers, page: state.page + 1, isLoadMore: false)),
      },
      (Failure failure) => {},
    );
  }
  ...
  Future<void> onRefreshAddBusiness(RefreshAddBusinessEvent event, Emitter emit) async {
    instance.get<AppLoadingBloc>().add(AppShowLoadingEvent());
    GetUserNotBusinessModel getUserNotBusinessModel = GetUserNotBusinessModel(search: state.txtSearch);
    final result = await userUseCase.getUsersNotBusiness(getUserNotBusinessModel);

    result.fold((List<UserEntity> users) => {emit(state.copyWith(users: users, usersChecked: [], page: 1))}, (r) => {});
    instance.get<AppLoadingBloc>().add(AppHideLoadingEvent());
  }
  ...
  Future<void> onAddCollaboratorAddBusiness(AddCollaboratorAddBusinessEvent event, Emitter emit) async {
    ...
    if (state.usersChecked.isEmpty) {
      emit(const AddCollaboratorFailedAddBusinessState(message: 'An error occurred. Please try again later'));
      ...
      if (result) {
        emit(const AddCollaboratorSuccessedAddBusinessState(message: 'Success'));
        add(RefreshAddBusinessEvent());
      } else {
        emit(state.copyWith());
      }
