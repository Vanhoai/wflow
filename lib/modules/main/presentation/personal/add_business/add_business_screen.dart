...
class _AddBusinessScreenState extends State<AddBusinessScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => AddBusinessBloc(userUseCase: instance.get<UserUseCase>())..add(InitAddBusinessEvent()),
        ...
        builder: (context, state) {
          _scrollController.addListener(() {
            if (_scrollController.position.maxScrollExtent == _scrollController.offset && !state.isLoadMore) {
              ...
              BlocProvider.of<AddBusinessBloc>(context).add(ScrollAddBusinessEvent());
            }
          });
            ...
            child: Scaffold(
              appBar: AppHeader(
                ...
                onTap: () => BlocProvider.of<AddBusinessBloc>(context).add(
                  AddCollaboratorAddBusinessEvent(usersChecked: state.usersChecked),
                ),
                actions: const [],
              ),
              body: SizedBox(
                    ...
                    SearchAddBusiness(
                      ...
                      onChangedSearch: (value) => {
                        BlocProvider.of<AddBusinessBloc>(context)
                            .add(ChangedIconClearAddBusinessEvent(txtSearch: value)),
                        if (_debounce?.isActive ?? false) _debounce?.cancel(),
                        _debounce = Timer(const Duration(milliseconds: 400), () {
                          BlocProvider.of<AddBusinessBloc>(context).add(SearchAddBusinessEvent(txtSearch: value));
                        }),
                      },
                      onClearSearch: () => {
                        _controller.clear(),
                        BlocProvider.of<AddBusinessBloc>(context)
                            .add(const ChangedIconClearAddBusinessEvent(txtSearch: '')),
                        BlocProvider.of<AddBusinessBloc>(context).add(const SearchAddBusinessEvent(txtSearch: '')),
                      },
                    ),
                    Expanded(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return AddBusinessCard(
                              ...
                              onCheck: (value) => BlocProvider.of<AddBusinessBloc>(context)
                                  .add(UserCheckedAddBusinessEvent(isChecked: value!, id: state.users[index].id)),
                            );
                          },
                        ),
                      ),
